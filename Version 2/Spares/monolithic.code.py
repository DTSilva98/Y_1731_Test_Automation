...
if version == 6: #Logic for Version 6 Ciena devices
    ...
    carrier = input('Please enter the Carrier initials (TMO, VZW, ATT, USC, BSP): ').strip().lower() #Get the carrier
    while carrier not in ['tmo','vzw','att','usc','bsp']:
        carrier = input('Invalid Entry, Please enter the Carrier initials (TMO, VZW, ATT, USC, BSP): ').strip().lower()
    ...
    servicename = "VLXP" + str(circuit) + ':' + str(vlan)  # Get and format the service name
